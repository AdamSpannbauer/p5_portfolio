...
class Cell {
  ...
  split() {
    const cell_sep = this.p2_delta - this.p1_delta;

    if (cell_sep <= this.max_sep) {
      this.p1_delta -= this.growrate;
      this.p2_delta += this.growrate;

      const new_x1 = this.x + cos(this.angle) * this.p1_delta;
      const new_x2 = this.x + cos(this.angle) * this.p2_delta;
      const new_y1 = this.y + sin(this.angle) * this.p1_delta;
      const new_y2 = this.y + sin(this.angle) * this.p2_delta;
  ...
  start_next_gen() {
    if (this.generation < max_gen) {
      ...
      const c1 = new Cell(this.p1.x, this.p1.y, this.r, this.max_r, this.angle + 90, this.generation + 1, null, this.fill1);
      this.children.push(c1);

      const c2 = new Cell(this.p2.x, this.p2.y, this.r, this.max_r, this.angle + 90, this.generation + 1, null, this.fill2);
